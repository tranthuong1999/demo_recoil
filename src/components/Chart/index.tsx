...
const DataTableVirtualized: React.FC<DataTableVirtualizedProps> = ({ data }) => {
  ...
  const Row: React.FC<ListChildComponentProps> = ({ index, style }) => {
    ...
    return (
      <div style={{ ...style, display: "flex", borderBottom: "1px solid #eee" }}>
        {columns.map(col => (
          <div key={col} style={{ flex: 1, padding: "0 8px" }}>{row[col] ?? ""}</div>
        ))}
      </div>
    );
  };
  ...
  return (
    <div style={{ border: "1px solid #ccc", borderRadius: 4 }}>
      <div style={{ display: "flex", fontWeight: "bold", background: "#f5f5f5" }}>
        {columns.map(col => (
          <div key={col} style={{ flex: 1, padding: "0 8px" }}>{col}</div>
        ))}
      </div>
      <List
        height={200}
...
const DenoChart = () => {
  ...
  const [selectedSheet, setSelectedSheet] = useState<SheetRow>(rowData[0]);
    ...
    <div>
      <div className="ag-theme-alpine" style={{ width: "100%" }}>
        <AgGridReact
          ...
          onRowClicked={params => {
            if (params.data && typeof params.data === 'object' && 'sheetName' in params.data) {
              setSelectedSheet(params.data as SheetRow);
            }
          }}
        />
      </div>
      {selectedSheet && (
        <div style={{ marginTop: 24 }}>
          ...
          <DataTableVirtualized data={selectedSheet.data} />
        </div>
      )}
    </div>
  );
